...
export function DayNameSection({ dayName, onDayNameChange }: DayNameSectionProps) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="font-serif text-base text-text-primary mb-2 block">
          Akan Day Name
        </label>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1">
          <label className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider">Date of Birth</label>
        ...
        <div className="w-full sm:w-36">
          <label className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider">Gender</label>
          <select
            value={gender}
            onChange={(e) => setGender(e.target.value as "male" | "female")}
            className="w-full rounded-sm border border-border bg-white px-3 py-3 text-sm text-text-primary focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors duration-200"
          >
        ...
        <div className="rounded-sm border border-border bg-white p-5 space-y-3">
          <div className="flex items-center gap-3">
            <div className="border-l-2 border-accent pl-3">
              <span className="text-2xl font-serif text-accent">{dayName.name}</span>
              <span className="text-sm text-text-tertiary ml-2">({dayName.day})</span>
            </div>
          </div>
          <p className="text-sm text-text-secondary leading-relaxed">{dayName.meaning}</p>
          <div className="flex flex-wrap gap-2">
            {dayName.traits.map((trait) => (
              <span
                key={trait}
                className="rounded-full bg-surface-raised border border-border px-2.5 py-0.5 text-xs text-text-secondary capitalize"
              >
                {trait}
              </span>
            ))}
